...
batch_size = 16
ds_train = WaveformDataset(x_train, y_train)
ds_val = WaveformDataset(x_test, y_test)
ds_eval = WaveformDataset(x_valid, y_valid)
...
eval_loader = DataLoader(ds_eval, batch_size=batch_size, shuffle=False, num_workers=4)
...
# Train
train(model, (train_loader, val_loader), device, save_dir)
...
try:
	...
	for key, ylabel in zip(
		...
		plt.plot(history[key], label=ylabel)
		plt.legend()
		plt.xlabel('Epoch')
		plt.ylabel(ylabel)
		plt.savefig(save_dir / f'{ylabel.replace(" ", "_").lower()}.png', dpi=200)
		plt.close()
