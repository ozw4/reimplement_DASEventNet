...
class GradCAM:
        ...
        def __init__(self, model: torch.nn.Module, target_layer: str = 'layer4') -> None:
                self.model = model
                self.model.eval()
        ...
        def __call__(self, x: torch.Tensor, class_idx: int | None = None) -> tuple[torch.Tensor, float]:
                """Return Grad-CAM map and prediction probability."""
                x = x.requires_grad_()
                output = self.model(x)
...
def visualize_gradcam(
        ...
        """Visualize Grad-CAM for a single waveform."""
